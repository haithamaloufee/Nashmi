import { CalendarDays, Users } from "lucide-react";
import { applicationTimeZone } from "@/lib/localization";

function formatDate(value?: string | Date | null) {
  if (!value) return "غير محدد";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "غير محدد";
  return date.toLocaleDateString("ar-JO", { year: "numeric", month: "short", day: "numeric", timeZone: applicationTimeZone });
}

export default function SurveyDateMeta({ startsAt, endsAt, totalResponses }: { startsAt?: string | Date | null; endsAt?: string | Date | null; totalResponses?: number | null }) {
  return (
    <div className="flex flex-wrap items-center gap-3 text-sm text-ink/65 dark:text-slate-300">
      <span className="inline-flex items-center gap-1.5">
        <CalendarDays className="h-4 w-4 text-civic" />
        {formatDate(startsAt)} - {formatDate(endsAt)}
      </span>
      {typeof totalResponses === "number" ? <span className="inline-flex items-center gap-1.5">
        <Users className="h-4 w-4 text-civic" />
        {totalResponses.toLocaleString("ar-JO")} مشارك
      </span> : null}
    </div>
  );
}
