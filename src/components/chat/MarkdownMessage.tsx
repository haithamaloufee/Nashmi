...
const markdownComponents: Components = {
  ...
  p: ({ children }) => <p className="my-2 leading-8 text-ink/90 dark:text-slate-100 first:mt-0 last:mb-0">{children}</p>,
  ul: ({ children }) => <ul className="my-2 list-disc space-y-2 pe-5 ps-0 leading-8 text-ink/90 dark:text-slate-100">{children}</ul>,
  ol: ({ children }) => <ol className="my-2 list-decimal space-y-2 pe-5 ps-0 leading-8 text-ink/90 dark:text-slate-100">{children}</ol>,
  ...
  a: ({ href, children }) => {
    ...
    return (
      <a
        ...
        className="text-civic underline underline-offset-4 dark:text-civic"
      >
        {children}
      </a>
    );
  ...
  code: ({ children }) => (
    <code className="rounded bg-slate-100 px-1 py-0.5 text-[0.92em] text-ink dark:bg-slate-900 dark:text-emerald-200">
      {children}
    </code>
  )
};
