...
export function classifyNewsSource(value: string): "official" | "news_agency" | "reputable_media" | null {
  ...
  if (hostname.endsWith(".gov.jo") || ["gov.jo", "pm.gov.jo", "parliament.jo", "senate.jo", "representatives.jo", "iec.jo", "parties.iec.jo", "ammancity.gov.jo"].some((domain) => hostname === domain || hostname.endsWith(`.${domain}`))) return "official";
  if (["almamlaka.tv", "alghad.com", "royanews.tv", "addustour.com", "jordantimes.com"].some((domain) => hostname === domain || hostname.endsWith(`.${domain}`))) return "reputable_media";
  return null;
}
