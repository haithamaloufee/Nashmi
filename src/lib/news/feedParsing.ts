...
export type FeedItem = { title: string; summary: string; url: string; publishedAt: string; publisher: string; sourceId: typeof FEEDS[number]["id"] };
...
export function parseFeedWithStats(xml: string, now: Date, feed: typeof FEEDS[number]) {
  ...
  return { items: items.slice(0, 100), rawFeedItems, afterAgeWindow };
}

export function parseFeed(xml: string, now: Date, feed: typeof FEEDS[number]) {
  return parseFeedWithStats(xml, now, feed).items;
}
